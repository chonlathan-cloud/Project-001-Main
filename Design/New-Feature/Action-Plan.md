...
เอกสารนี้เป็นแผนพัฒนา code สำหรับ Inspection feature โดยอิงจาก `UX-Flow.md` และ decision ที่ confirm แล้ว

## 1. Confirmed Decisions

- ใช้ role เดิมของระบบ: Admin / Owner / Inspector / Subcontractor
- Subcontractor เห็นเฉพาะ defect ที่ assign ให้ตัวเอง
- Subcontractor เปลี่ยน status ได้สูงสุดถึง `Ready for Review`
- Frontend ไม่ยิง Firestore/GCS โดยตรง ทุกอย่างผ่าน FastAPI backend
- Inspection ผูกกับ `projectId` เดิมเสมอ
- 1 project มีหลาย inspection round ได้ และ defect ต้องผูกกับ round เสมอ
- Zone / floor / room สร้างเองใน Inspection เพื่อความยืดหยุ่น
- GCS ใช้ private bucket + signed URL
- เก็บไฟล์: before photo, after photo, plan image, PDF report ในอนาคต
- MVP report ใช้ frontend print/PDF ก่อน ยังไม่ generate PDF แล้วเก็บลง GCS
- เก็บ report log เบื้องต้น เช่น user, time, project, round, report type
- Notification ยังไม่ทำใน MVP แต่เผื่อ event model ไว้สำหรับ Gmail ในอนาคต
...
เหตุผลที่ให้ backend เป็น gateway:

- ใช้ auth/session และ permission logic เดิมของระบบ
- ป้องกันไม่ให้ frontend ถือ credential ของ GCP
- คุม signed URL expiration ได้จาก backend
- ทำ audit timeline และ status transition validation ได้ชัด
- ภายหลังต่อ Gmail notification ได้จาก backend event เดียวกัน

## 3. Proposed Firestore Data Model

ใช้ top-level collections เพื่อ query ง่ายทั้งระดับ project, round และ subcontractor
...
```json
{
  ...
  "description": "ตรวจงานก่อนส่งมอบรอบแรก",
  "status": "ACTIVE",
...
```json
{
  ...
  "description": "พบคราบน้ำและหยดน้ำบริเวณฝ้า",
  "category": "Plumbing",
  "severity": "CRITICAL",
  "status": "OPEN",
...
- ไม่มี status `REOPENED`
- ถ้า inspector ตรวจไม่ผ่าน ให้เปลี่ยนจาก `READY_FOR_REVIEW` กลับเป็น `IN_PROGRESS`
- บันทึก timeline event เป็น `REVIEW_REJECTED` หรือ `REOPENED`
...
Private bucket, object access through signed URL only
...
`reports` เก็บไว้สำหรับ future phase
...
- Inspector/Admin/Owner can move `READY_FOR_REVIEW` back to `IN_PROGRESS` with review rejection comment
...
- After photo: Owner/Admin/Inspector/Subcontractor if assigned to defect
...
Add local section/tab inside `ProjectDetailPage`:
...
MVP can render `InspectionWorkspace` inside Project Detail when active tab is `inspection`
...
- Full-width map
- Drawer becomes bottom sheet or full-screen panel
...
- `READY_FOR_REVIEW -> RESOLVED` only Inspector/Admin/Owner
- `READY_FOR_REVIEW -> IN_PROGRESS` requires rejection comment
- Subcontractor can only move assigned defect to `READY_FOR_REVIEW`
- Every status transition writes `inspection_events`
...
- Start with count-based score for predictability
- Surface blockers separately: open critical, overdue, ready-for-review waiting approval
...
- Finalize role naming for Inspector if role does not exist yet
- Confirm GCS bucket env var name
- Confirm signed URL expiration default
...
- Add GCS inspection file service using existing private/signed URL pattern
- Add schemas for rounds, zones, defects, files, events, report logs
...
- Pin appears at correct coordinate
...
- Log contains user, project, round, report type, filters, timestamp
...
Keep these out of MVP but design the data so they can be added:

- Gmail notification for assignment, overdue, ready for review, rejected review
- Saved PDF report in GCS
...
- File privacy: signed URL must be generated only after permission check
- Status integrity: frontend must not be trusted; backend must validate transitions
- Report print logs: frontend print cannot guarantee actual print completion, only print intent
- Role model: `Inspector` needs clear mapping if not already present in auth/session
